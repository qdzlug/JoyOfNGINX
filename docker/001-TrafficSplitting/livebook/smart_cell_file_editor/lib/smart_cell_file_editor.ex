defmodule SmartCellFileEditor do
  ...
  def init(attrs, ctx) do
    IO.inspect(attrs, label: :init_attrs)
    {
      :ok,
      assign(
        ctx,
  ...
  def handle_connect(ctx) do
    IO.inspect(ctx, label: :handle_connect)
  ...
  @impl true
  def to_attrs(ctx) do
    IO.inspect(ctx, label: :to_attrs)
    %{"file_content" => ctx.assigns.file_content, filepath: ctx.assigns.filepath}
  end

  @impl true
  def to_source(attrs) do
    IO.inspect(attrs, label: :to_source_attrs)
    # TODO: This is not working - the file path doesn't stay on
    quote do
